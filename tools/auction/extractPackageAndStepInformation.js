const extract = (auction) => {
    ...
    if (auction.options.includes('MULTISTEP')) {
        ...
        for (let i = 0; i < nbrPackages; i++) {
            const pkg = auction.dimension.slice(i * packageLength, (i + 1) * packageLength);
        ...
        for (let i = 0; i < nbrPackages; i++) {
            ...
            const addressA = pkg.slice(dimensionsLength, dimensionsLength + addressLength);
            const addressB = pkg.slice(dimensionsLength + addressLength, packageLength);
            ...
            if (steps.find(e => e.id === idA) === undefined) {
                steps.push({
                    id: idA,
                    key: addressA[0].length > 1 ? addressA[0]: `${auction.key}-${addressA[0]}`,
                    date: addressA[12],
            ...
            if (steps.find(e => e.id === idB) === undefined) {
                steps.push({
                    id: idB,
                    key: addressB[0].length > 1 ? addressB[0]: `${auction.key}-${addressB[0]}`,
                    date: addressB[12],
        ...
        if (auction.options.includes('PKG_V1')) {
            for (let i = 0; i < auction.dimension.length / 6; i++) {
                ...
                const stackableMatch = auction.dimension[5 + offset].match(/stackable:[\s]*(\d)/m);
